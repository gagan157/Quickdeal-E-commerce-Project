  ...
  if (data) {
    ...
    if (dataLoad && localStorage.getItem("cart")) {
      ...
      for (let data of dataInfo) {
        // cartinlist(data[0])
        TotalAmount +=
          Number(dataInfo.get(data[0]).price) *
          Number(dataInfo.get(data[0]).count);
       
        //hide btn
        let cartbtnmain = document.getElementById(`prd-btn-${data[0]}`)
        if(cartbtnmain){
          cartbtnmain.style.display = "none";
        }
        //show incree and drement btn
        let cartbtn = document.getElementById(`prd-btn-addcart-${data[0]}`)
        if(cartbtn){
          cartbtn.style.display = "flex";
        }  

        //show count
        let btncount = document.getElementById(`prd-incti-count-${data[0]}`)
        if(btncount){
          btncount.innerText = dataInfo.get(data[0]).count;
        }  
...
async function fetchOneCategoryData() {
  ...
  // console.log(data)
  if (response) {
    document.getElementsByClassName("catageryWisePrd-loader")[0].style.display =
      "none";
  }
  Show(data.products);
}
function Show(data) {
  ...
  for (let da of data) {
    divcatwiseprd.innerHTML += `<div class="catwise-card" id="in-Spotlight-${da.id}">
    ...
    <div class="catwise-card-button">
      <div id="prd-btn-${da.id}">
        <button onClick={addToCart(this)} id="${da.id}" type="button">Add to Cart</button>
      </div>
      <div id="prd-btn-addcart-${da.id}" class="prd-btn-addcart" style="display: none; gap:5px; align-items: center;">
        <button onClick={decrimentprd(this)} id="decri-${da.id}" class="decriment"><i class="fa-solid fa-minus"></i></button>
        <span id="prd-incti-count-${da.id}">0</span>
        <button onClick={incrimentprd(this)} id="incri-${da.id}" class="inciment"><i class="fa-sharp fa-solid fa-plus"></i></button>
      </div>
    </div>
    </div>`;
  }
...
function getdata(e) {
  ...
  Promise.all([data]).then((result) => {
    ...
    let IsCartInItem =  new Map(JSON.parse(localStorage.getItem("cart")));

    result[0].forEach((element) => {
      divcatwiseprd.innerHTML += `<div class="catwise-card" id="in-Spotlight-${element.id}">
        <div class="catwise-card-img">
        ...
        <div class="catwise-card-button">        
          <div id="prd-btn-${element.id}">            
            ${IsCartInItem.has(`${element.id}`)?`<button style="display:none;" onClick={addToCart(this)} id="${element.id}" type="button">Add to Cart</button>`:`<button onClick={addToCart(this)} id="${element.id}" type="button">Add to Cart</button>`}
          </div>
          ${  
            IsCartInItem.has(`${element.id}`)?
            `<div id="prd-btn-addcart-${element.id}" class="prd-btn-addcart" style="display: flex; gap:5px; align-items: center;">
            <button onClick={decrimentprd(this)} id="decri-${element.id}" class="decriment"><i class="fa-solid fa-minus"></i></button>
            <span id="prd-incti-count-${element.id}">${IsCartInItem.get(`${element.id}`)?.count}</span>
            <button onClick={incrimentprd(this)} id="incri-${element.id}" class="inciment"><i class="fa-sharp fa-solid fa-plus"></i></button>
            </div>`
            :
            `<div id="prd-btn-addcart-${element.id}" class="prd-btn-addcart" style="display: none; gap:5px; align-items: center;">
            <button onClick={decrimentprd(this)} id="decri-${element.id}" class="decriment"><i class="fa-solid fa-minus"></i></button>
            <span id="prd-incti-count-${element.id}">0</span>
            <button onClick={incrimentprd(this)} id="incri-${element.id}" class="inciment"><i class="fa-sharp fa-solid fa-plus"></i></button>
            </div>` 
      ...
      //changes if any item in cart then remove main btn and show incri and dcri btn other wise not
...
function addToCart(e) {
  ...
  if(!prdCard){
    prdCard = document.getElementById(`in-Spotlight-${e.id}`)
  }
  ...
  if(!price){
    price = prdCard.children[1].children[0].innerText.split(' ')[0].split('$')[1]
  }
  ...
  if (!localStorage.getItem("cart")) {
    ...
    localStorage.setItem(
      "cart",
      JSON.stringify(Array.from(datainfo.entries()))
    );
  } else {
    let dataInfo = new Map(JSON.parse(localStorage.getItem("cart")));
    dataInfo.set(e.id, {
